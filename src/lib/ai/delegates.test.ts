...
describe("buildDelegationTools", () => {
  ...
  it("exposes every member-accessible delegate skill to members", () => {
    const tools = buildDelegationTools(contextForRole(UserRole.Member), new TurnUsageTracker());
    expect(Object.keys(tools).sort()).toEqual(expectedDelegateNames(UserRole.Member));
  });
